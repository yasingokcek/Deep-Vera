...
const packages = [
  { id: '1', tokens: 100, price: '100 TL' },
  { id: '2', tokens: 500, price: '500 TL', popular: true },
  { id: '3', tokens: 1000, price: '1000 TL' },
];
      ...
      <div className="bg-white max-w-2xl w-full rounded-[3rem] overflow-hidden flex flex-col p-10 lg:p-14 shadow-2xl border border-slate-100 text-left">
        <div className="flex justify-between items-start mb-10">
           <div className="flex flex-col">
              <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Kredi Satın Al</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">1 Kredi = 1.00 TL</p>
           </div>
        ...
        <div className="flex gap-4 mb-8 bg-slate-50 p-1.5 rounded-2xl">
           <button onClick={() => setActiveTab('tokens')} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl ${activeTab === 'tokens' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}>Kredi Ekle</button>
        ...
        {activeTab === 'tokens' ? (
          <div className="space-y-6">
            <div className="space-y-3">
              {packages.map((pkg) => (
                <button key={pkg.id} onClick={() => setSelectedPkg(pkg)} className={`w-full p-6 rounded-2xl border flex items-center justify-between ${selectedPkg.id === pkg.id ? 'bg-blue-50 border-blue-600' : 'bg-white border-slate-100'}`}>
                  <div className="flex flex-col items-start">
                     <span className={`text-[12px] font-black uppercase tracking-widest ${selectedPkg.id === pkg.id ? 'text-blue-600' : 'text-slate-900'}`}>{pkg.tokens.toLocaleString()} AI-Kredi</span>
            ...
            <button onClick={() => onSuccess(selectedPkg.tokens)} className="w-full py-6 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl">Ödemeyi Tamamla</button>
          </div>
        ) : (
               ...
               <div className="relative z-10">
                  <div className="flex justify-between items-start mb-6">
                     ...
                     <div className="text-right">
                        <span className="text-3xl font-black text-slate-900">990 TL</span>
                  ...
                  <ul className="space-y-4 mb-10 text-xs font-bold text-slate-600">
                     <li>✓ Tüm kredilerde %50 indirim</li>
